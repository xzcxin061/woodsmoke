...
//     ];
    ...
    public function maximumDrawdown ()
    {
        ...
        if (!empty($this->handleFinalArr)) {
            ...
            foreach ($this->handleFinalArr as $key => &$value) {
                ...
                if ($key == "20".$this->stopTime) {
                    ...
                    // 投资时间平移的最大回撤率
                    $childrenArr = array_slice(array_column($this->handleFinalArr, 'shouyie'), $keysReverseArr[$key], $size - $keysReverseArr[$key], true);
        ...
        for ($i = 0; $i < count($latestDayArr) - 1; $i++) {
            $latestDayArr[0]['shouyilvAverage'] = 0.0000; // 凑数用的
